...
$order = get_order_details($user_id, $_SESSION['order_id']);

// Check if there was an error retrieving the order details
if ($order === NULL) {
    Logger::getInstance()->error('[ERROR] Failed to fetch order details.', ['userid' => $user_id, 'order_id' => $_SESSION['order_id']]);
...
if (empty($order)) {
    Logger::getInstance()->error('[ORDER] Order details not found.', ['userid' => $user_id, 'order_id' => $_SESSION['order_id']]);
